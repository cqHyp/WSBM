...
public class NewsFoodAdapter extends BaseFooterAdapter<News> {
    public interface OnItemClickListener{
        void itemClick(View view,int position,int nid);
    }
    private OnItemClickListener onItemClickListener;
    private View footer;
    public NewsFoodAdapter(Context context, List<News> data,View footer) {
        super(context, data);
        this.footer=footer;
    }
    ...
    public BaseItem initItem(ViewGroup parent) {
        View view=getInflater().inflate(R.layout.item_news_food,parent,false);
        return new MyItem(view);
    }

    @Override
    public void bindItem(BaseAdapter.BaseItem item, final int position) {
        ((MyItem) item).title.setText(getData().get(position).getTitle());
